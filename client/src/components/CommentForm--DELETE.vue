          ...
          <v-card color="surface" height="auto">
               ...
               <v-card-text>
                    <v-row>
                         ...
                         <v-col class="v-col-6">
                              <v-form>
                                   <v-text-field block v-model="input.name" label="Your name (optional)"></v-text-field>
...
//import { interpolatePuOr } from 'd3';
...
const comment = computed(() => redditStore.currentComment)
watch(comment, (newComment) => {
     input.comment = newComment.body
     input.name = newComment.author
})
